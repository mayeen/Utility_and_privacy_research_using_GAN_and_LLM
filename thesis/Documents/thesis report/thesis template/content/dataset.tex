...
generators and eleven single-number metrics. Cross-dataset generalisation is discussed as
...
The integer fields include encounter and patient identifiers, utilisation counts, procedure counts,
...
\texttt{?} is treated as a missing value, and known count and identifier
...
clinical and administrative fields: \texttt{weight} (96.86\%),
\texttt{max-glucose-serum} (94.75\%), \texttt{A1Cresult} (83.28\%), \texttt{medical specialty}
(49.08\%), and \texttt{payer code} (39.56\%), with lower missingness in \texttt{race}
(2.23\%), \texttt{diag\_3} (1.40\%), \texttt{diag\_2} (0.35\%), and
\texttt{diag\_1} (0.02\%). The engineering response to these patterns
(field removal versus imputation) is described in
...
severe class imbalance: the dominant value \texttt{No} accounts for more
than 99\% of encounters in each, leaving fewer than 1\% non-modal
observations. Among the affected fields are rare agents such as
\texttt{examide}, \texttt{citoglipton}, \texttt{troglitazone}, and several
combination medications. Rare-category collapse is a documented failure
mode for tabular generators~\cite{Xu2019CTGAN}; the corresponding encoding
...
\texttt{<30} for 11{,}357 encounters (11.16\%). The two readmission
...
\paragraph{Summary.}
Together these properties: high missingness in a small set of
administrative fields, severe imbalance in rare-medication indicators, and
a multi-class readmission target requiring binary collapse define the
design constraints addressed by the engineering pipeline of
Section~\ref{sec:data:fe}.
...
(1)~multi-encounter resolution, (2)~field removal, (3)~age encoding,
...
are resolved (\texttt{process\_multiple\_encounters}) by sorting on
\texttt{patient\_nbr} and \texttt{time\_in\_hospital}, retaining the
encounter with the longest hospital stay, and dropping duplicate patient
rows. \texttt{patient\_nbr} is then removed as a direct patient identifier,
and \texttt{race} is removed to limit reliance on a sensitive demographic
field.

\paragraph{Field removal.}
Three high-missingness fields: \texttt{weight} (96.86\%),
\texttt{payer\_code} (39.56\%), and \texttt{medical\_specialty} (49.08\%)
...
The decade-based age buckets are converted to ordered numeric values: 
\path{[0-10)} maps to 1, \path{[10-20)}
to 2, and so on through \texttt{[90-100)}, which maps to 10. The 1--10
...
The three diagnosis fields \texttt{diag\_1}, \texttt{diag\_2}, and
\texttt{diag\_3} together contain 915 distinct ICD-9 codes in the raw
dataset. These are mapped to broader diagnosis groups
(\texttt{change\_diag\_columns}, \texttt{\_icd9\_to\_group}): codes
beginning with 250 are mapped to \texttt{Diabetes}, and numeric ICD-9
...
supplementary codes beginning with \texttt{E} or \texttt{V}, missing
values, and unrecognised codes are mapped to \texttt{Other}. The grouping
is a clinically motivated taxonomy inspired by Strack et
al.~\cite{strack2014impact} and is not derived from the standard CCS /
...
The 23 medication columns are converted to binary exposure indicators
(\texttt{change\_medication\_columns}): \texttt{No} is encoded as 0, and
every other observed value (including \texttt{Steady}, \texttt{Up}, and
\texttt{Down}) is encoded as 1. The binary representation records whether
...
The \texttt{A1Cresult} field is converted to an ordinal laboratory
indicator (\texttt{encode\_a1cresult}). Missing A1C values are encoded as
0, with the remaining levels mapped as \texttt{Norm}~$\rightarrow$~1,
\texttt{>7}~$\rightarrow$~2, and \texttt{>8}~$\rightarrow$~3. The 0 level
...
Remaining missing values are imputed (\texttt{impute\_data}): categorical
columns by the most frequent observed value and numeric columns by the
median, both via \texttt{SimpleImputer}. After imputation, the
preprocessed dataset contains no missing values. All remaining
object-type feature columns are then converted to integer labels via
\texttt{LabelEncoder} (\texttt{label\_encode}), excluding the target.

\paragraph{Target binarisation.}
The original three-class readmission variable is collapsed to a binary
outcome (\texttt{target\_encode}): \texttt{NO} is encoded as 0, while both
\texttt{>30} and \texttt{<30} are encoded as 1. This any-readmission
formulation follows the standard convention adopted in earlier work on
the same dataset~\cite{strack2014impact} and is used consistently
throughout downstream training and evaluation.
...
The preprocessing script writes three CSV files: the full preprocessed
dataset (71{,}518 rows, 45 columns), the training split, and the test
split. The split is created with \texttt{train\_test\_split},
\texttt{test\_size = 0.2}, and \texttt{random\_state = 42}, with the
target column passed as the stratification variable to preserve the
binary readmission ratio across splits. Because
\path{process_multiple_encounters} produces one row per
\texttt{patient\_nbr}, the stratified split is inherently patient-disjoint
