...
WGAN-GP objective, in which a gradient penalty regulates the critic. A DP-augmented HealthGAN variant would
...
DP-SGD provides privacy by computing a parameter gradient for each training
record, clipping each gradient to limit the influence of that record, and
adding Gaussian noise to the aggregated gradient. A privacy accountant then
...
critic~\cite{Yousefpour2021Opacus}. However, retaining the gradient-penalty
term would have required per-example differentiation through the critic's
input-gradient calculation, which was not supported by the chosen
implementation. The prototype therefore removed the gradient penalty and used
...
generator, a student discriminator, and an ensemble of teacher discriminators.
...
feature-engineered table. Each of the $D=45$ columns in the preprocessed training split is min-max
...
The student and generator are both optimised with Root Mean Square Propagation (RMSProp) at learning
...
sequence with a class-label prefix. It then fine-tunes the base model on those sequences using low-rank
adaptation. To generate data, it prompts the trained model with the same
class prefix and parses the generated text back into typed values under the
source schema. The subsubsections below describe the base model, the row-to-text
serialisation, the LoRA fine-tuning configuration, the class-conditioned generation
procedure, and the schema-aware postprocessing that derives a per-column
schema and converts free-form model output into a validated synthetic
dataframe.
...
postprocessing of generated outputs. For every column, the schema records 
the original pandas dtype, whether the column is numeric, and, 
for numeric columns, the observed minimum and maximum. It also records 
...
the schema stores its full sorted discrete support, which covers ordinal-coded 
columns such as age decile and diagnosis groupings as well as binary medication 
...
lineup~\cite{Li2022LargeDP}. The LLM-side DP toggle is operationalised at the 1B scale, while
...
The pipeline follows the parameter-efficient framework where the base Pythia weights are frozen and
...
respect to the private training corpus~\cite{Yu2022DPFT}. The AdamW optimiser, restricted
...
parameter-efficient fine-tuning~\cite{Yu2022DPFT}. Both
