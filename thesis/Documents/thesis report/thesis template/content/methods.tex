...
health records. It further introduces Nearest-Neighbour Adversarial Accuracy,
a metric suite that jointly quantifies resemblance and privacy, and reports
competitive utility and resemblance relative to established baselines while
supporting secure model export~\cite{healthgan}. These properties make it a
...
The benchmark study of \textcite{Miletic2024} evaluates the Pythia family of
decoder-only language models for synthetic tabular health data using the
Tabula framework, in which each table row is serialised as a text sequence,
and reports that these models results are comparable to CTGAN on
train-on-synthetic, test-on-real utility. Pythia-70M is included here as the
...
\paragraph{Pythia-1B:}
A larger non-private LLM variant is added so that the LLM-side DP comparison
(Pythia-1B versus Pythia-1B-DP) isolates the cost of DP-SGD at a fixed model
size, rather than confounding it with the model-scale change to 1B. Pythia-1B
...
Floating-point columns are scaled to the interval $[0,1]$ using min--max
normalisation. The original minimum and maximum of each column are stored so
that the transformation can be inverted during decoding.
...
Synthetic files evaluated & $3 \times 57{,}214$ records; \\
  & plus $1 \times 14{,}304$ test-sized draw \\
\bottomrule
\end{tabular}
\end{table}
